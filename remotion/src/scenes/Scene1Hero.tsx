...
export const Scene1Hero = () => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
      <div
        style={{
          width: 90, height: 90, borderRadius: "50%", border: "3px solid hsl(38, 92%, 50%)",
          display: "flex", alignItems: "center", justifyContent: "center",
          transform: `scale(${logoScale}) rotate(${frame * 0.5}deg)`,
          marginBottom: 20, background: "hsla(38, 92%, 50%, 0.1)",
        }}
      >
        <div style={{ fontSize: 40, color: "hsl(38, 92%, 50%)" }}>⚙</div>
      </div>
      <div style={{ fontFamily: spaceGrotesk, fontSize: 72, fontWeight: 700, color: "white", opacity: titleOp, transform: `translateY(${titleY}px)`, textAlign: "center", lineHeight: 1.1 }}>
        Novo <span style={{ color: "hsl(38, 92%, 50%)" }}>PeçaI</span>
      </div>
      <div style={{ fontFamily: inter, fontSize: 28, color: "hsla(0, 0%, 100%, 0.7)", opacity: subtitleOp, transform: `translateY(${subtitleY}px)`, marginTop: 16, textAlign: "center" }}>
        Plataforma Completa para o Setor Automotivo
      </div>
      <div style={{ width: lineW, height: 3, background: "linear-gradient(90deg, transparent, hsl(38, 92%, 50%), transparent)", marginTop: 30, borderRadius: 2 }} />
      <div style={{ opacity: tagOp, fontFamily: inter, fontSize: 18, color: "hsl(38, 92%, 50%)", marginTop: 24, padding: "8px 24px", border: "1px solid hsla(38, 92%, 50%, 0.3)", borderRadius: 30, background: "hsla(38, 92%, 50%, 0.08)" }}>
        Substitua planilhas e ERPs genéricos
      </div>
      <div style={{ display: "flex", gap: 80, marginTop: 50, opacity: statsOp }}>
        <StatItem label="Módulos" value={`${count1}+`} />
        <StatItem label="Veículos" value={`${count2}+`} />
        <StatItem label="Plano Anual" value="R$ 297" />
      </div>
      <Subtitle text="NovoPeçaI: a plataforma completa para autopeças. Vendas, estoque, CRM e muito mais." from={40} />
    </AbsoluteFill>
  );
};
...
const StatItem = ({ label, value }: { label: string; value: string }) => (
  <div style={{ textAlign: "center" }}>
    <div style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: 40, fontWeight: 700, color: "white" }}>{value}</div>
    <div style={{ fontFamily: "Inter, sans-serif", fontSize: 16, color: "hsla(0,0%,100%,0.5)", marginTop: 4 }}>{label}</div>
  </div>
);
