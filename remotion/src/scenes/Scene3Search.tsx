...
export const Scene3Search = () => {
  ...
  const charsShown = Math.min(Math.floor(interpolate(frame, [20, 70], [0, searchText.length], {
  ...
  return (
    <AbsoluteFill style={{ padding: 80 }}>
      ...
      {/* Results */}
      <Sequence from={75}>
        <ResultsList frame={frame - 75} fps={fps} />
      </Sequence>
    </AbsoluteFill>
  );
};
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {results.map((r, i) => {
        ...
        return (
          ...
          >
            <div>
              <div style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: "hsl(38, 92%, 50%)", marginBottom: 4 }}>
                {r.code} • {r.brand}
              </div>
              <div style={{ fontFamily: "Inter, sans-serif", fontSize: 20, color: "white", fontWeight: 500 }}>
                {r.name}
              </div>
            </div>
            <div style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: 24, fontWeight: 700, color: "#22c55e" }}>
              {r.price}
            </div>
          </div>
        );
